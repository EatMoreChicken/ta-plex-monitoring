...
def collect_events(helper, ew):
    ...
    # Construct URL to make the request for sessions history
    url = f"http://{plex_server}:{plex_port}/status/sessions/history/all?X-Plex-Token={plex_token}"

    # Make the GET request and store the response
    response = requests.get(url)
    response_xml = ET.fromstring(response.content)

    # Loop through the history elements in the XML and create an event for each one
