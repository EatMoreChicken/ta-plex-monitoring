...
def collect_events(helper, ew):
    ...
    # Construct URL to make the request for library sections
    url = f"http://{plex_server}:{plex_port}/library/sections/?X-Plex-Token={plex_token}"

    # Make the GET request and store the response
    response = requests.get(url)
    response_xml = ET.fromstring(response.content)

    # Loop through the sections in the XML and create an event for each one
