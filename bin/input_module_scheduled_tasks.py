...
def collect_events(helper, ew):
    ...
    # Construct URL to make the request for scheduled tasks
    url = f"http://{plex_server}:{plex_port}/butler?X-Plex-Token={plex_token}"

    # Make the GET request and store the response
    response = requests.get(url)

    # Parse the response XML and loop through the child elements
    response_xml = ET.fromstring(response.content)
    for child in response_xml:
        # Convert the child element to a string
